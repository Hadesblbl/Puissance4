...
public class Grille {
	...
	public double evaluer(Case symboleJoueurCourant)
	{
		...
		double evaluation=0;
		int nbAlignes=0;
		boolean alignementPossible=true;
		double[] scoreAlignes = {0,100,100*100,100*100*100,100*100*100*100};
		if(symboleJoueurCourant==Constantes.SYMBOLE_J1)
		{
		...
		//Evaluation des alignements possibles pour nous
		...
		for(int i=0;i<Constantes.NB_LIGNES;i++)
		{
			for(int j=0;j<Constantes.NB_COLONNES-3;j++)
			{
				for(int x=0;x<4;x++)
				{
					...
					{
						alignementPossible=false;
					}
					if(grille[j+x][i]==symboleJoueurCourant) 
					{
						nbAlignes++;
					}
				}
				if (alignementPossible)
				{
					evaluation+=scoreAlignes[nbAlignes];
				}
				nbAlignes=0;
				alignementPossible=true;
			}
		}
		...
		for(int i=0;i<Constantes.NB_LIGNES-3;i++)
		{
			for(int j=0;j<Constantes.NB_COLONNES;j++)
			{
				for(int x=0;x<4;x++)
				{
					...
					if(grille[j][i+x]==symboleJoueurCourant) 
					{
						nbAlignes++;
					}
				}
